...
const StudentTranscript = () => {
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            // ── ألوان السيستم ──
            const COLOR_DARK = [30, 41, 59];        // #1e293b - header الجداول
            const COLOR_BLUE = [37, 99, 235];        // #2563eb
            const COLOR_GREEN = [16, 185, 129];      // #10b981
            const COLOR_RED = [239, 68, 68];         // #ef4444
            const COLOR_AMBER = [245, 158, 11];      // #f59e0b
            const COLOR_LIGHT_BG = [241, 245, 249];  // #f1f5f9
            const COLOR_MUTED = [100, 116, 139];     // #64748b
            const COLOR_WHITE = [255, 255, 255];
            const COLOR_SUMMARY_BG = [248, 250, 252];// #f8fafc
            ...
            leftData.forEach(([label, value]) => {
                ...
                // لو الاسم عربي نعرضه بشكل مناسب
            ...
            // Academic Status badge inside box
            ...
            doc.text(
                `Academic Alerts — Consecutive: ${alerts}/4   |   Total: ${totalAlerts}/6   |   Advisor: ${advisor?.staffName || "Not Assigned"}`,
                margin + 3, y + 6.5
            );
            y += 16;
            ...
            if (semesterWorks && semesterWorks.length > 0) {
                ...
                doc.setFont(undefined, "bold");
                doc.setTextColor(...COLOR_DARK);
                doc.text(`Current Semester Works  —  ${semester?.name || ""}`, margin, y);
                y += 2;
            ...
            // جمّع الكورسات بالسيميستر (نفس منطق الـ UI)
            ...
            for (const sem of sortedSems) {
                ...
                autoTable(doc, {
                    ...
                    // Override style للصف الأول (semester label)
                    didParseCell(hookData) {
                        ...
                        if (hookData.section === "body") {
                            ...
                            const isSummaryRow = rowIndex === courses.length; // آخر row = summary
